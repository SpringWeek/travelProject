...
object HbaseTools extends Logging with Serializable {
  ...
  def getOffsetFromHBase(connection: Connection, admin: Admin, topics: Array[String], group: String): collection.Map[TopicPartition, Long] = {
    if (!admin.tableExists(TableName.valueOf(Constants.HBASE_OFFSET_STORE_TABLE))) {
      val hbaseoffsetstoretable: TableDescriptorBuilder = TableDescriptorBuilder.newBuilder(TableName.valueOf(Constants.HBASE_OFFSET_STORE_TABLE))
      val descriptor: ColumnFamilyDescriptor = ColumnFamilyDescriptorBuilder.newBuilder(Constants.HBASE_OFFSET_FAMILY_NAME.getBytes()).build()

      val descriptor1: TableDescriptor = hbaseoffsetstoretable.setColumnFamily(descriptor).build()
